...
function [TargetFitness,TargetPosition,Convergence_curve]=GOA(N, Max_iter, lb,ub, dim, fobj)
...
while l<Max_iter+1
    ...
    for i=1:size(GrassHopperPositions,1)
        ...
        for k=1:2:dim
            ...
            for j=1:N
                if i~=j
                    Dist=distance_GOA(temp(k:k+1,j), temp(k:k+1,i)); % Calculate the distance between two grasshoppers
